...
#### 📝 Markdown 编辑器
- **工具栏按钮**：加粗、斜体、标题、引用、代码、链接、列表、图片
- **实时预览**：左侧编辑，右侧实时渲染
- **完整语法支持**：支持 GFM（GitHub Flavored Markdown）
- **优化样式**：美观的 Markdown 渲染样式
...
#### 共享日记管理
- ✅ 新增"我的共享"标签页：用户可查看和管理所有自己创建的共享日记
- ✅ 显示共享日记的详细信息（标题、分类、分享模式、评论数）
- ✅ 支持查看共享日记的完整内容摘要
- ✅ 可以直接从"我的共享"页面查看或删除共享日记
- ✅ 共享码显示，方便用户分享给朋友

#### 删除级联功能
- ✅ 删除个人日记时，同时删除 Firebase 中对应的共享副本
- ✅ 删除级联时同时清理 localStorage 中的映射记录
- ✅ 用户友好的确认提示

#### 共享模式增强
- ✅ 改进共享模式显示：明确区分"所有人可见"（公开）和"朋友可见"（需要码）
- ✅ SharedPlaza 中的共享日记卡片显示清晰的共享模式标签
- ✅ 支持同时显示公开和朋友可见的日记（带不同的视觉标识）

#### 删除共享功能
- ✅ 在共享日记详情页添加删除按钮
- ✅ 用户可以删除自己创建的共享日记
- ✅ 删除时同时清理 Firebase 和 localStorage 数据

### 🔧 Bug 修复
- ✅ 修复 shareMode 值的一致性（'private' → 'friends'）
- ✅ 增强共享日记的创建者身份验证
